...
class ScoringEngine {
    static calculateScore(student, event, interactions = []) {
        ...
        const studentSkillNames = (student.studentSkills || student.skills || []).map((s) => (s.skill?.name || s.skillName || s).toLowerCase() || '').filter(Boolean);
        const eventRequiredSkills = typeof event.requiredSkills === 'string' ? event.requiredSkills.split(',') : (event.requiredSkills || []);
        const eventSkillNames = eventRequiredSkills.concat(intelligence.skills || []).map((s) => s.toLowerCase());
        let skillScore = 0;
        if (eventSkillNames.length > 0) {
            const matchedSkills = studentSkillNames.filter((s) => eventSkillNames.some((es) => es.includes(s) || s.includes(es)));
            ...
            const missing = eventSkillNames.filter((es) => !studentSkillNames.some((s) => es.includes(s) || s.includes(es)));
            if (missing.length > 0) {
        ...
        const studentInterests = (student.interests || []).map((i) => (i.interest?.name || i.name || i).toLowerCase() || '').filter(Boolean);
        const eventDomains = (intelligence.domains || []).concat([event.category || '']).map((d) => d.toLowerCase());
        let interestScore = 0;
        if (studentInterests.length > 0 && eventDomains.length > 0) {
            const matchedInterests = studentInterests.filter((interest) => eventDomains.some((domain) => domain.includes(interest) || interest.includes(domain)));
        ...
        if (student.careerGoal) {
            ...
            const careerPaths = (intelligence.careerPaths || []).map((cp) => cp.toLowerCase());
            const isGoalMatch = careerPaths.some((cp) => cp.includes(goalLower) || goalLower.includes(cp)) ||
        ...
        const totalScore = branchScore * weights.branchMatch +
