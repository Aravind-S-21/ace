...
export class CalendarService {
  ...
  public async addCalendarEvent(data: {
    ...
    if (reminderType === 'EVENT_START') {
      reminderTime = new Date(new Date(event.startDate).getTime() - 24 * 60 * 60 * 1000); // 1 day before event start
  ...
  public async getStudentCalendar(studentId: string): Promise<any[]> {
