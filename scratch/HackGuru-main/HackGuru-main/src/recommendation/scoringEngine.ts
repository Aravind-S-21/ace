  ...
  public static calculateScore(student: any, event: any, interactions: any[] = []): CandidateEventScore {
    ...
    if (student.branch) {
      ...
      if (isTechBranch && isTechEvent) {
        branchScore = 1.0;
        matchingSignals.push(`Strong academic branch match (${student.branch}) for ${event.category}`);
      } else if (event.eligibility.toLowerCase().includes(studentBranchLower) || event.eligibility.toLowerCase().includes('all')) {
        branchScore = 0.8;
        matchingSignals.push(`Eligible for ${student.branch} students`);
      }
    }

    // 2. Skill Overlap Score (Jaccard Similarity / Weighted overlap)
    const studentSkillNames = (student.skills || []).map((s: any) => s.skill?.name?.toLowerCase() || '').filter(Boolean);
    const eventSkillNames = (event.requiredSkills || []).concat(intelligence.skills || []).map((s: string) => s.toLowerCase());
    ...
    const studentInterests = (student.interests || []).map((i: any) => i.interest?.name?.toLowerCase() || '').filter(Boolean);
    ...
    if (event.location.toLowerCase().includes('online') || event.location.toLowerCase().includes('remote')) {
      locationScore = 1.0;
      matchingSignals.push('Flexible online / remote participation');
    } else if (student.location && event.location.toLowerCase().includes(student.location.toLowerCase())) {
      locationScore = 1.0;
    ...
    const eventInteractions = interactions.filter((i) => i.eventId === event.id);
    const hasDismissed = eventInteractions.some((i) => i.action === 'DISMISS');
    const hasSaved = eventInteractions.some((i) => i.action === 'SAVE' || i.action === 'CALENDAR_ADD');
    const hasRegistered = eventInteractions.some((i) => i.action === 'REGISTER');
