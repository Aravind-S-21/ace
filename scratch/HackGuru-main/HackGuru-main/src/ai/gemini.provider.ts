...
export class GeminiProvider implements AIProvider {
  ...
  private getGenAI(key: string): GoogleGenerativeAI {
...
Return EXACTLY a JSON object matching this schema:
{
  ...
  "eventType": "HACKATHON" | "WORKSHOP" | "INTERNSHIP" | "SEMINAR" | "COMPETITION" | "PROJECT_COLLAB" | "OTHER"
    ...
    for (const cred of this.credentials) {
      try {
        const genAI = this.getGenAI(cred.secret);
        ...
        return {
          ...
          metrics: {
            ...
            inputTokens: result.response.usageMetadata?.promptTokenCount || 200,
            outputTokens: result.response.usageMetadata?.candidatesTokenCount || 100,
            totalTokens: result.response.usageMetadata?.totalTokenCount || 300,
            estimatedCost: (result.response.usageMetadata?.totalTokenCount || 300) * 0.000001,
            durationMs,
            success: true,
          },
        };
      ...
      try {
        ...
        return {
          ...
          metrics: {
            ...
            outputTokens: result.response.usageMetadata?.candidatesTokenCount || 300,
            totalTokens: result.response.usageMetadata?.totalTokenCount || 800,
            estimatedCost: (result.response.usageMetadata?.totalTokenCount || 800) * 0.000001,
            durationMs,
            success: true,
          },
        };
